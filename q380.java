...
class q380 {
    ...
    HashSet<Integer> removedElements;
    ...
    public boolean insert(int val) {
        if (ss.contains(val))
            return false;
        al.add(val);
        if (removedElements.contains(val))
            removedElements.remove((Integer) val);
        return ss.add(val);
    }
    ...
    public int getRandom() {
        int element = getRandomElement(al);
        // System.out.println(removedElements);
        while (removedElements.contains(element)) {
            // System.out.print(pos+","+al+","+removedElements+"~~");
            element = getRandomElement(al);
            if (al.size() == 1)
                break;
        }
        return element;
...
/*
 * Runtime
 * 49ms Beats 41.25% of users with Java
 * 
 * Memory
 * 94.12MB Beats 8.44% of users with Java
 */