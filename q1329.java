...
class q1329 {
    ...
    public int[][] diagonalSort(int[][] arr) {
        ...
        for (j = 0; j < m; j++) {
            int t = j, traverse = 0;
            lst = new ArrayList<>();
            for (int k = 0; k < m && check(k, t); k++, t++)
                lst.add(arr[k][t]);
            Collections.sort(lst);
            t = j;
            for (int k = 0; k < m && check(k, t); k++, t++)
                arr[k][t] = lst.get(traverse++);
            lst = null;
        }
        // print(arr);
        for (j = 0; j < n; j++) {
            int t = j, traverse = 0;
            lst = new ArrayList<>();
            for (int k = 0; k < n && check(t, k); k++, t++)
                lst.add(arr[t][k]);
            Collections.sort(lst);
            t = j;
            for (int k = 0; k < n && check(t, k); k++, t++)
                arr[t][k] = lst.get(traverse++);
            lst = null;
        }
        return arr;
    }
...
/*
 * Q1329. Sort the Matrix Diagonally
 * 
...
/*
 * Runtime
 * 9ms Beats 45.25% of users with Java
 * 
 * Memory
 * 44.73MB Beats 41.06% of users with Java
 */